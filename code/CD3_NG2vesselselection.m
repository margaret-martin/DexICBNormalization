function CD3_NG2vesselselection( rootPath,action )

%Channel 1 is CD3 and channel 2 is aSMA
modCD3=1;  modNG2=2;  moddapi=3;
modCt = 3;
...
if strcmp(action,'step1')
    ...
    for p=1:numel(paths)
        ...
        for f=1:numel(files)
            ...
            if contains(data_name,' CD31 ') && ~contains(data_name,'NG2')
                filename=fullfile(matPath,sprintf('%s.mat',data_name));
...
%                     nonaSMA_areafrac=length(nonaSMA_size)/tot_pixels;
                    ...
                    %SAVE
                    vesseldensity=[vesseldensity vessel_d];
...
%                 name = [name;string(curr_name)];
    ...
    csvname = fullfile(csvPath,'vesseldensity.csv');
    vesseldensity=transpose(vesseldensity);
...
%get asma bw 
if strcmp(action,'NG2')
    paths=dirPaths(dataPath);
    nonvesselareafrac_ng2=[];
    name = [];
    for p=1:numel(paths)
        path=fullfile(dataPath,paths(p).name);
        files=dir(fullfile(path));
        ...
        for f=1:numel(files)
            data_name = strrep(files(f).name,'.bmp','');
            if contains(data_name,'NG2') && ~contains(data_name,'CD31')
                filename=fullfile(matPath,sprintf('%s.mat',data_name));
                a=strfind(data_name,'NG2');
                loadname=strcat('mat/',data_name(1:a-1),' CD31 ',data_name(a+3:end),'.mat');
                load(loadname);
                vesselbw=bw;
                data = imread(fullfile(path,files(f).name));
                data = data(:,:,modNG2);
                ...
                nonvesselareafrac_ng2=[nonvesselareafrac_ng2 nonvessel_areafrac];
                curr_name = char(data_name);
                %             filename = strcat(curr_name(1:spaces(1)),curr_name(spaces(end-1):end));
                %             name = [name;string(filename)];
                name = [name;string(curr_name)];
    ...
    nonvesselareafrac_ng2=transpose(nonvesselareafrac_ng2);
    % CD3_areafraction=transpose(CD3_areafraction);
    % aSMA_celldensity=transpose(aSMA_celldensity);
    % CD3_celldensity=transpose(CD3_celldensity);
    T = table(name,nonvesselareafrac_ng2);
    writetable(T,csvname);
